      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-10 relative z-10">
        ...
        <div className="relative w-full">
          ...
          <div className={`grid grid-cols-1 gap-6 md:gap-8 mx-auto ${photos.length === 1 ? 'max-w-[486px]' : photos.length === 2 ? 'md:grid-cols-2 max-w-[800px]' : 'md:grid-cols-3'}`}>
            {photos.length > 0 ? photos.slice(0, 3).map((foto, idx) => (
              <div key={idx} className="relative flex flex-col items-center justify-center bg-[#F8FAFD] rounded-[32px] p-3 hover:shadow-xl transition-all duration-300 group cursor-pointer w-full h-[400px] md:h-[486px]">
