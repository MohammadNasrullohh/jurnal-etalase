...
def take_screenshot():
    with sync_playwright() as p:
        ...
        # Ketik 4 digit PIN (kursor akan maju otomatis karena fungsi handlePinChange)
        print("Mengetik PIN 1-2-3-4 secara berurutan...")
        page.locator("input[type='password']").nth(0).click()
        page.keyboard.type("1234", delay=200)

        # Tunggu proses redirect otomatis (timeout diperpanjang)
        print("Menunggu redirect otomatis ke Panel Arsip...")
        page.wait_for_url("http://localhost:3000/panel", timeout=30000)

        print("Mengklik tab 'Jurnal Saya'...")
        page.get_by_role("button", name="Jurnal Saya Lihat riwayat jurnal").click()
        page.wait_for_timeout(1000)

        print("Mengambil screenshot halaman hasil redirect...")
        # Screenshot area layar penuh
        page.screenshot(path="hasil_screenshot_terbaru.png")
        
        print("Selesai! Screenshot berhasil disimpan sebagai 'hasil_screenshot_terbaru.png'")
        browser.close()
